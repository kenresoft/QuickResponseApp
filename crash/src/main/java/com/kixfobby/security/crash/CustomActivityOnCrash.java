...
package com.kixfobby.security.crash;
...
import com.kixfobby.security.crash.activity.DefaultErrorActivity;

import static com.kixfobby.security.crash.config.CaocConfig.BACKGROUND_MODE_SHOW_CUSTOM;
...
public class CustomActivityOnCrash {
    ...
    private static final String EXTRA_CONFIG = "com.kixfobby.security.crash.EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = "com.kixfobby.security.crash.EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = "com.kixfobby.security.crash.EXTRA_ACTIVITY_LOG";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.kixfobby.security.crash.ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.kixfobby.security.crash.RESTART";
    private static final String CAOC_HANDLER_PACKAGE_NAME = "com.kixfobby.security.crash";
    ...
    private static com.kixfobby.security.crash.config.CaocConfig config = new com.kixfobby.security.crash.config.CaocConfig();
                        ...
                        public void uncaughtException(@NonNull Thread thread, @NonNull final Throwable throwable) {
                            if (config.isEnabled()) {
                                ...
                                if (hasCrashedInTheLastSeconds(application)) {
                                    ...
                                    } else if (config.getBackgroundMode() == com.kixfobby.security.crash.config.CaocConfig.BACKGROUND_MODE_CRASH) {
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
     ...
     */
    @Nullable
    public static com.kixfobby.security.crash.config.CaocConfig getConfigFromIntent(@NonNull Intent intent) {
        com.kixfobby.security.crash.config.CaocConfig config = (com.kixfobby.security.crash.config.CaocConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull com.kixfobby.security.crash.config.CaocConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull com.kixfobby.security.crash.config.CaocConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull com.kixfobby.security.crash.config.CaocConfig config) {
        if (config.getEventListener() != null) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static com.kixfobby.security.crash.config.CaocConfig getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull com.kixfobby.security.crash.config.CaocConfig config) {
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.kixfobby.security.crash.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.kixfobby.security.crash.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.kixfobby.security.crash.ERROR" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.kixfobby.security.crash.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
