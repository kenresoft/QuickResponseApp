...
@HiltViewModel
class NearbyVM @Inject constructor(application: Application, private val appDatabase: AppDatabase) :
    ...
    private fun checkIsPremiumPurchased(v: View): Boolean {
        ...
        if (/*!isPurchased && */!NetworkManager.isOnline(v.context)!!) {
        ...
        private fun checkIsPremiumPurchased(v: View): Boolean {
            val isPurchased = if (isPremiumPurchased.value != null) isPremiumPurchased.value!! else false
            if (!isPurchased && !com.kixfobby.security.quickresponse.helper.network.NetworkManager.isOnline(v.context)!!) {
                BaseActivity.showSnackBar(v, R.string.err_no_internet)
                return false
            }
